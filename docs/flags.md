...
| `--txt-apex-replacement=""` | When using the TXT registry, a custom string that's used as subdomain for TXT records instead of the apex domain; Uses domains defined by txt-apex-domain, with fallback to domain-filter (optional) |
| `--txt-apex-domain=TXT-APEX-DOMAIN` | Apex domain to use when txt-apex-replacement is defined; specify multiple times for multiple domains (optional) |
